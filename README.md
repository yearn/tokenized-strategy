> [!CAUTION]
> **v3.1.0 risk:** share pricing is driven by `strategyTotalAssets()`. BaseStrategy defaults this to `lastTotalAssets()`, preserving v3.0.4 report-boundary accounting. Strategies that override it for live accounting can move PPS, previews, and conversions before `report()`; if that value is wrong or manipulable, vault shares can be mispriced.
