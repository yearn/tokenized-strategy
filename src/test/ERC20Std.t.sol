...
contract ERC20BaseTest is Setup {
    ...
    function invariant_metadata() public {
        ...
        assertEq(strategy.symbol(),   "tsSTGY");
    ...
    function testFuzz_decreaseAllowance_nonInfiniteApproval(address account_, uint256 initialAmount_, uint256 subtractedAmount_) public {
        ...
        assertEq(strategy.allowance(self, account_), initialAmount_ - subtractedAmount_);
    }
...
contract ERC20PermitTest is Setup {
    ...
    function test_permit_zeroAddress() public {
        ( uint8 v, bytes32 r, bytes32 s ) = _getValidPermitSignature(address(strategy), _owner, _spender, 1000, 0, _deadline, _skOwner);
    ...
    function test_permit_differentSpender() public {
        ( uint8 v, bytes32 r, bytes32 s ) = _getValidPermitSignature(address(strategy), _owner, address(1111), 1000, 0, _deadline, _skOwner);
    ...
    function test_permit_ownerSignerMismatch() public {
        ( uint8 v, bytes32 r, bytes32 s ) = _getValidPermitSignature(address(strategy), _owner, _spender, 1000, 0, _deadline, _skSpender);
    ...
    function test_permit_withExpiry() public {
        ...
        ( uint8 v, bytes32 r, bytes32 s ) = _getValidPermitSignature(address(strategy), _owner, _spender, 1000, 0, expiry, _skOwner);
    ...
    function test_permit_earlyNonce() public {
        ( uint8 v, bytes32 r, bytes32 s ) = _getValidPermitSignature(address(strategy), _owner, _spender, 1000, 1, _deadline, _skOwner);
    ...
    function test_permit_badS() public {
        ( uint8 v, bytes32 r, ) = _getValidPermitSignature(address(strategy), _owner, _spender, 1000, 0, _deadline, _skOwner);
        ...
        return keccak256(
            abi.encodePacked(
                ...
                keccak256(abi.encode(keccak256(
                                ...
                                ), owner_, spender_, amount_, nonce_, deadline_))
            )
        );
    }
...
contract ERC20User {
    ...
    function erc20_permit(
        ...
        external
    {
        IStrategy(token_).permit(owner_, spender_, amount_, deadline_, v_, r_, s_);
    }
