...
import {LegacyMockIlliquidStrategy, LegacyMockStrategy} from "./mocks/LegacyMockStrategy.sol";
import {Setup} from "./utils/Setup.sol";

contract LegacyAccountingTest is Setup {
    function setUp() public override {
        super.setUp();
        vm.label(address(strategy), "legacy strategy");
    }

    function setUpStrategy() public override returns (address) {
        IMockStrategy _strategy = IMockStrategy(
            address(
                new LegacyMockStrategy(address(asset), address(yieldSource))
            )
        );

        _setUpRoles(_strategy);

        return address(_strategy);
    }

    function setUpIlliquidStrategy() public override returns (address) {
        IMockStrategy _strategy = IMockStrategy(
            address(
                new LegacyMockIlliquidStrategy(
                    address(asset),
                    address(yieldSource)
                )
            )
    ...
    function _setUpRoles(IMockStrategy _strategy) internal {
